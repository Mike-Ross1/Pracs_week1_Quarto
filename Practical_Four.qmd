...
As seen in the code below, carriers which have only 1 flight have NA standard 
deviation and carriers who fly the same distance have a 0 standard deviation.
...
Dataframe where the carriers are along the columns, and the rows are the average 
departure delay flown by each carrier in each month.
...
The route with the greatest difference between the best and worst performing
airlines.
...
have better scheduling and processes than the worst which allow them to depart 
on time. Another possible reason may be that the worst performing airline was 
scheduled during bad weather or had more technical issues than the best which 
caused a delay.

## Question 7

As seen in the output below, gender has a spelling mistake (femal), 
height has NA values, disease_status has both "healthy" and "Healthy" 
categories, and finally glucose has NA values.
```{r, echo=FALSE}
data_q7 = structure(list(id = c("id_1", "id_2", "id_3", "id_4", "id_5", 
